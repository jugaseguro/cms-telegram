...
export async function POST(request: Request) {
  ...
  // --- Campaign tracking: create campaign record ---
  ...
  // --- Campaign tracking: collect recipient records ---
  type RecipientEntry = {
    campaign_id: string
    customer_id: string
  ...
  for (let i = 0; i < customers.length; i += BATCH_SIZE) {
    ...
    for (const { customer, result } of batchResults) {
      const telegramMsgId = (result.data.result as any)?.message_id ?? null

      if (result.ok) {
        sentCount++
        sentEntries.push({
          conversationId: customer.conversationId,
          telegram_message_id: telegramMsgId?.toString() ?? null,
        })
      } else {
        failedCount++
      }

      // Track recipient regardless of success/failure
      if (campaignId) {
        recipientEntries.push({
          campaign_id: campaignId,
          customer_id: customer.id,
          conversation_id: customer.conversationId,
          telegram_message_id: telegramMsgId ? Number(telegramMsgId) : null,
  ...
  // --- Campaign tracking: bulk insert recipients and update campaign totals ---
  if (campaignId) {
    if (recipientEntries.length > 0) {
      await supabase.from('mass_message_recipients').insert(recipientEntries)
    }
